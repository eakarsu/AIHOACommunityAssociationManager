...
test('deterministic assessment fails closed and keeps a human approval boundary', () => {
  ...
  const result = workflow.deterministicAssessment({ ownershipVersion:'deed-v2', governingDocumentVersion:'rules-v7', noticeDeadline:'2099-01-02T00:00:00Z', evaluatedAt:'2099-01-01T00:00:00Z', quorumRequired:4, quorumPresent:5, policyVersion:'policy-v3' }, actor(config.assessmentRoles[0]));
  assert.equal(result.disposition, 'eligible_for_board_review');
  assert.equal(result.automatedDecision, false);
  assert.equal(result.requiresHumanReview, true);
  expectCode(() => workflow.deterministicAssessment({ ownershipVersion:'deed-v2', governingDocumentVersion:'rules-v7', noticeDeadline:'2099-01-02T00:00:00Z', evaluatedAt:'2099-01-01T00:00:00Z', quorumRequired:4, quorumPresent:5, policyVersion:'policy-v3' }, actor('unauthorized_role')), 'FORBIDDEN');
});
