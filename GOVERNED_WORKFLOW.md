# Governed association official-record workflow

## Scope

The new endpoint is `/api/governed-association-records`. It implements ownership-linked notices, board review, quorum evidence, appeals, and immutable official-record history. The state sequence is:

`draft → noticed → owner_response → board_review → approved/appealed → resolved → recorded`
...
- Case identity, evidence, and events are protected from destructive updates/deletes by database constraints and triggers. The event history is readable at `GET /api/governed-association-records/cases/:id/history`.
- Deterministic triage validates ownership and governing-document versions, notice deadline, explicit evaluation time, quorum, ledger/ballot/minutes digests. It always returns `automatedDecision: false` and `requiresHumanReview: true`.
- The workflow supports documented association procedure; it does not determine legal rights or replace association counsel.
